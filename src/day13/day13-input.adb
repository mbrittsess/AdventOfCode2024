...
package body Day13.Input is
   function Read_Machine_Input ( List : Machine_List := [] ) return Machine_List is
      ...
      A_X : Positive := Positive'Value(A_Line(13..14));
      A_Y : Positive := Positive'Value(A_Line(19..20));
      A_Pos : Vector := (A_X, A_Y);

      B_X : Positive := Positive'Value(B_Line(13..14));
      B_Y : Positive := Positive'Value(B_Line(19..20));
      B_Pos : Vector := (B_X, B_Y);

      Comma_Pos : Positive := Ada.Strings.Fixed.Index(Prize_Line, ",");
      P_X : Positive := Positive'Value(Prize_Line(10..(Comma_Pos-1)));
      P_Y : Positive := Positive'Value(Prize_Line((Comma_Pos+4)..Prize_Line'Last));
